...
class EPD extends Display {
  ...
  initDisplay() {
    return new Promise(async (resolve, reject) => {
      try {
        // reset GPIO and get ready for accepting commands
        await GPIOWrapper.output(PINS.CS, GPIOWrapper.LOW); // not required?
        ...
        // EPD7IN5 configure
        await this.writeData(COMMANDS.POWER_SETTING);
        await this.writeData(COMMANDS.PANEL_SETTING);
        await this.writeData(COMMANDS.BOOSTER_SOFT_START);
        await this.writeData(COMMANDS.POWER_ON);
        await this.writeData(COMMANDS.PLL_CONTROL);
        await this.whileBusy();
        await this.writeData(COMMANDS.TEMPERATURE_CALIBRATION);
        await this.writeData(COMMANDS.VCOM_AND_DATA_INTERVAL_SETTING);
        await this.writeData(COMMANDS.TCON_SETTING);
        await this.writeData(COMMANDS.TCON_RESOLUTION);
        await this.writeData(COMMANDS.VCM_DC_SETTING);
        await this.writeData(COMMANDS.FLASH_MODE);
